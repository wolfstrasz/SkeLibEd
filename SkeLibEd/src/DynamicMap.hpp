...
public:
	...
	private:
		...
		public:
			...
			// Constructors
			ThreadArgument() {}
			ThreadArgument(std::vector<OUT> &output, std::vector<IN> &input, size_t threadInputIndex, size_t chunkSize, bool isFinished)
				: threadInputIndex(threadInputIndex), chunkSize(chunkSize), input(&input), output(&output),isFinished(isFinished) {}
			// Destructor
			~ThreadArgument() {
		...
		void threadMap(Scoreboard *scoreboard, ThreadArgument<IN, OUT> *threadArguments, size_t threadID, ARGs... args) {
			...
			while (!scoreboard->isFinished) {
				if (threadArguments[threadID].isThereWork) {
					...
					scoreboard->finishedWorkers.push_back(threadID);
				}
			}
		...
		DynamicMapImplementation(Elemental<EL> elemental, size_t threads, size_t sizeOfChunk) : elemental(elemental) {
			this->nthreads = nthreads ? nthreads : std::thread::hardware_concurrency();
		...
		void operator()(std::vector<OUT> &output, std::vector<IN> &input, ARGs... args) {
			...
			while (!scoreboard->isFinished) {
				...
				if (!scoreboard.finishedWorkers.empty()) {
					size_t threadID = scoreboard.finishedWorkers.pop_front();
					...
					if (scoreboard->curIndex + scoreboard->itemsCount < scoreboard->inputSize) {
						...
						threadArguments[t].elementsCount = lastItemsCount;
						threadArguments[t].threadInputIndex = scoreboard->curIndex;
				...
				if (scoreboard.inputSize == scoreboard.curIndex)
